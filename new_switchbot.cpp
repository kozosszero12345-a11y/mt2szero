...
void CSwitchbot::SetAttributes(BYTE slot, std::vector<TSwitchbotAttributeAlternativeTable> vec_alternatives)
{
	...
	if (vec_alternatives.size() < SWITCHBOT_ALTERNATIVE_COUNT)
	{
		sys_err("SetAttributes> not enough alternatives given (size %d, expected %d)", vec_alternatives.size(), SWITCHBOT_ALTERNATIVE_COUNT);
		return;
	}
		...
		else
		{
			...
			if (SWITCHBOT_PRICE_TYPE == 1)
			{
				for (const auto& itemVnum : c_arSwitchingItems)
				{
					...
					LPITEM pkCandidate = pkOwner->FindSpecifyItem(itemVnum);
					if (pkCandidate && pkCandidate->GetCount() >= SWITCHBOT_PRICE_AMOUNT)
					{
						stop = false;
						pkPriceItem = pkCandidate;
						break;
					}
				}
			}
			...
			if (SWITCHBOT_PRICE_TYPE == 1)
			{
				if (pkPriceItem)
				{
					pkPriceItem->SetCount(pkPriceItem->GetCount() - SWITCHBOT_PRICE_AMOUNT);
				}
			}
			else if (SWITCHBOT_PRICE_TYPE == 2)
			{
